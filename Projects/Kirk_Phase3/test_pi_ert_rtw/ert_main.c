...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(test_pi_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(test_pi_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(test_pi_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(test_pi_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(test_pi_M, true);
    }
  }
